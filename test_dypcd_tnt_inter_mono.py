...
conf_all = {'Family':0.1, 'Francis':0.1, 'Horse':0.075, 'Lighthouse':0.05, 'M60':0.2, 'Panther':0.15, 'Playground':0.05, 'Train':0.2}
dist_all = {'Family':1/1*d_r['Family'], 'Francis':1/1*d_r['Francis'], 'Horse':1/1*d_r['Horse'], 'Lighthouse':1/1*d_r['Lighthouse'],
            'M60':1/1*d_r['M60'], 'Panther':1/1*d_r['Panther'], 'Playground':1/1*d_r['Playground'], 'Train':1/1*d_r['Train']}
rel_diff_all = {'Family':1/5000*r_r['Family'], 'Francis':1/4000*r_r['Francis'], 'Horse':1/3000*r_r['Horse'], 'Lighthouse':1/6500*r_r['Lighthouse'],
                'M60':1/2500*r_r['M60'], 'Panther':1/2000*r_r['Panther'], 'Playground':1/6000*r_r['Playground'], 'Train':1/2500*r_r['Train']}
...
parser.add_argument('--use_trans', type=str, default='not_use_trans')
...
def check_geometric_consistency(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src,scan):
    ...
    # mask_ = np.logical_and(dist < 4 * dist_base, depth_diff < 10 * rel_diff_base)
    # return masks, mask_, depth_reprojected, x2d_src, y2d_src
...
def save_scene_depth(testlist):
    ...
    if args.dataset == 'tanks' or args.dataset == 'tanks_mono':
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            for filename, cam, img, depth_est, photometric_confidence in zip(filenames, cams, imgs, \
                                                            outputs["maskup_depth"], outputs["photometric_confidence"]):
                # print(depth_est.shape)
                img = img[0].numpy()  #ref view
                cam = cam[0]  #ref cam
                depth_filename = os.path.join(args.outdir, filename.format('depth_est', '.pfm'))
                confidence_filename = os.path.join(args.outdir, filename.format('confidence', '.pfm'))
                confidence_filename1 = os.path.join(args.outdir, filename.format('confidence_1', '.pfm'))
                confidence_filename2 = os.path.join(args.outdir, filename.format('confidence_2', '.pfm'))
                confidence_filename3 = os.path.join(args.outdir, filename.format('confidence_3', '.pfm'))
                cam_filename = os.path.join(args.outdir, filename.format('cams', '_cam.txt'))
                img_filename = os.path.join(args.outdir, filename.format('images', '.jpg'))
                ply_filename = os.path.join(args.outdir, filename.format('ply_local', '.ply'))
                os.makedirs(depth_filename.rsplit('/', 1)[0], exist_ok=True)
                os.makedirs(confidence_filename.rsplit('/', 1)[0], exist_ok=True)
                os.makedirs(confidence_filename1.rsplit('/', 1)[0], exist_ok=True)
                os.makedirs(confidence_filename2.rsplit('/', 1)[0], exist_ok=True)
                os.makedirs(confidence_filename3.rsplit('/', 1)[0], exist_ok=True)
                os.makedirs(cam_filename.rsplit('/', 1)[0], exist_ok=True)
                os.makedirs(img_filename.rsplit('/', 1)[0], exist_ok=True)
                os.makedirs(ply_filename.rsplit('/', 1)[0], exist_ok=True)
                #save depth maps
                save_pfm(depth_filename, depth_est)
                # depth_jpg_filename = os.path.join(args.outdir, filename.format('depth_est', '.jpg'))
