...
parser.add_argument('--use_trans', type=str, default='use_trans')
parser.add_argument('--trans', type=str, default='src')
parser.add_argument('--visi_train', type=str, default='not_visi_train')
# network
...
def train(model, model_loss, optimizer, TrainImgLoader, TestImgLoader, start_epoch, args):
    ...
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            if (not is_distributed) or (dist.get_rank() == 0):
                if do_summary:
                    ...
                    print(
                       "Epoch {}/{}, Iter {}/{}, lr {:.6f}, train loss = {:.3f}, ce_loss = {:.3f}, {:.3f}, {:.3f}, {:.3f}, maskdepth = {:.3f}, {:.3f}, {:.3f}, {:.3f}, epe:{:.6f}, err3:{:.6f}, err1:{:.6f},time = {:.3f}".format(
                           ...
                           scalar_outputs["s0_maskdepth"],
                           scalar_outputs["s1_maskdepth"],
                           scalar_outputs["s2_maskdepth"],
                           scalar_outputs["s3_maskdepth"],
                           scalar_outputs['epe'],
                           scalar_outputs['err3'],
                           scalar_outputs['err1'],
        ...
        model_save = model.module.state_dict()

        for key in list(model_save):
            if key.startswith("mono_pretrain_model"):
                del model_save[key]
        ...
        if (epoch_idx % args.eval_freq == 0) or (epoch_idx == args.epochs - 1):
            ...
            for batch_idx, sample in enumerate(TestImgLoader):
                ...
                if (not is_distributed) or (dist.get_rank() == 0):
                    if do_summary:
                        print(
                            "Epoch {}/{}, Iter {}/{}, lr {:.6f}, test loss = {:.3f}, c_loss = {:.3f}, {:.3f}, {:.3f}, {:.3f}, maskdepth = {:.3f}, {:.3f}, {:.3f}, {:.3f}, epe:{:.6f}, err3:{:.6f}, err1:{:.6f}, time = {:.3f}".format(
...
def train_sample(model, model_loss, optimizer, sample, args):
    ...
    visi_gt_ms = sample_cuda["visi_masks"]
    outputs = model(sample_cuda["imgs"], sample_cuda["ref_img"], sample_cuda["proj_matrices"], sample_cuda["depth_values"])
    depth_est = outputs["depth"]
    depth_est_mask_up = outputs["maskup_depth"]
    loss, stage_ce_loss, range_err_ratio, epe, err3, err1, stage_mask_up_loss = model_loss(
    ...
    scalar_outputs = {"loss": loss,
                      ...
                      "s0_maskdepth":stage_mask_up_loss[0],
                      "s1_maskdepth":stage_mask_up_loss[1],
                      "s2_maskdepth":stage_mask_up_loss[2],
                      "s3_maskdepth":stage_mask_up_loss[3],
                      "abs_depth_error": AbsDepthError_metrics(depth_est_mask_up, depth_gt, mask > 0.5),
                      "thres2mm_error": Thres_metrics(depth_est_mask_up, depth_gt, mask > 0.5, 2),
                      "thres4mm_error": Thres_metrics(depth_est_mask_up, depth_gt, mask > 0.5, 4),
                      "thres8mm_error": Thres_metrics(depth_est_mask_up, depth_gt, mask > 0.5, 8),
                      'epe':epe,
                      'err3':err3,
                      'err1':err1}
...
def test_sample_depth(model, model_loss, sample, args):
    ...
    outputs = model_eval(sample_cuda["imgs"], sample_cuda["ref_img"], sample_cuda["proj_matrices"], sample_cuda["depth_values"])
    depth_est = outputs["depth"]
    depth_est_mask_up = outputs["maskup_depth"]
    loss, stage_ce_loss, range_err_ratio, epe, err3, err1, stage_mask_up_loss = model_loss(
                                        outputs, depth_gt_ms, mask_ms, stage_lw=[float(e) for e in args.dlossw.split(",") if e],
...
if __name__ == '__main__':
    ...
    # model, optimizer
    model = EC_MVSNet(arch_mode=args.arch_mode, reg_net=args.reg_mode, num_stage=4,
    ...
    if args.dataset.startswith('dtu'):
        ...
        train_dataset = MVSDataset(args.trainpath, args.trainlist, "train", view_num, robust_train=args.rt)
        test_dataset = MVSDataset(args.testpath, args.testlist, "val", view_num)
    if is_distributed:
