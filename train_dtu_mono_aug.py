    ...
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            if (not is_distributed) or (dist.get_rank() == 0):
                if do_summary:
                    print(
                        "Epoch {}/{}, Iter {}/{}, lr {:.6f}, test loss = {:.3f}, mask_loss = {:.3f}, {:.3f}, {:.3f}, {:.3f}, ce_loss = {:.3f}, {:.3f}, {:.3f}, {:.3f}, range_err = {:.3f}, {:.3f}, {:.3f}, {:.3f}, time = {:.3f}".format(
                            epoch_idx, args.epochs, batch_idx, len(TrainImgLoader),
                            optimizer.param_groups[0]["lr"],
                            loss,
                            scalar_outputs["s0_maskup_loss"],
                            scalar_outputs["s1_maskup_loss"],
                            scalar_outputs["s2_maskup_loss"],
                            scalar_outputs["s3_maskup_loss"],
...
def train_sample(model, model_loss, optimizer, sample, args):
    ...
    loss, stage_ce_loss, range_err_ratio, stage_mask_up_loss, _ = model_loss(
    ...
    scalar_outputs = {"loss": loss,
                      ...
                      "s0_maskup_loss": stage_mask_up_loss[0],
                      "s1_maskup_loss": stage_mask_up_loss[1],
                      "s2_maskup_loss": stage_mask_up_loss[2],
                      "s3_maskup_loss": stage_mask_up_loss[3],
                      ...
                      "maskup_abs_depth_error": AbsDepthError_metrics(depth_est_mask_up, depth_gt, mask > 0.5),
                      "maskup_thres2mm_error": Thres_metrics(depth_est_mask_up, depth_gt, mask > 0.5, 2),
                      "maskup_thres4mm_error": Thres_metrics(depth_est_mask_up, depth_gt, mask > 0.5, 4),
                      "maskup_thres8mm_error": Thres_metrics(depth_est_mask_up, depth_gt, mask > 0.5, 8),
                      }
...
if __name__ == '__main__':
    ...
    if is_distributed:
        ...
        TrainImgLoader = DataLoader(train_dataset, args.batch_size, shuffle=False, num_workers=8,
